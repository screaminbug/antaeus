 ...
 */
internal fun getNextMonthlyScheduledDateForStartDay(startDay: Int): Date {
    val now = getInstance()
    return getNextMonthlyScheduledDateForStartDay(
        currentYear = now.get(YEAR),
        currentMonth = now.get(MONTH) + 1,  // Calendar months start from 0
        currentDay = now.get(DAY_OF_MONTH),
        startDay = startDay
    )
}
...
internal fun getNextMonthlyScheduledDateForStartDay(currentYear: Int, currentMonth: Int, currentDay: Int, startDay: Int): Date {
    ...
    if (currentDay < startDayThisMonth) {
        fromWhen.set(DAY_OF_MONTH, startDayThisMonth)
    ...
    // to not overflow to the next month
        ...
        if (currentDay > startDayNextMonth) { fromWhen.set(DAY_OF_MONTH, startDayNextMonth) }
        fromWhen.add(MONTH, 1)
    }
